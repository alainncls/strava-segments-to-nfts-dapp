  ...
  return (
    <Row>
      <Col>
        ...
        {segment.metadata && (
          <a href={segment.metadata} target={"_blank"} rel="noreferrer" className={"text-decoration-none"}>
      ...
      <Col>
        <>
          {segment.metadata ? (
            <Button size={"sm"} onClick={() => mintNft()} disabled={!isConnected}>
      ...
      {segment.picture && <img alt={`Segment ${segment.id}`} src={segment.picture} width={500} className={"my-2"} />}
    </Row>
  );
};
